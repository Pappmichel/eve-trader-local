...
|---|---|
| `eve_trader/goonmetrics_client.py` | Region price/history fetch shape, the caching *pattern* (not the cache itself — see the parent repo's CLAUDE.md "Caching pattern" section). Port `esi_client.structure_order_stats_bulk_or_goonmetrics` at the same time — it was deliberately skipped in the esi_client port above because it's the one method that depends on this client. |
