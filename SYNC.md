...
| `eve_trader/models.py` (the rest), `eve_trader/*/models.py` | Plain dataclasses — portable as-is where they don't reference `storage`/`TRADING_CONFIG` directly. `Candidate` and `NewCandidateResult` are ported; `ShortlistItem`/`ShortlistRow`/`RealizedTrade`/… belong to modules still listed here, so each arrives with its own |
