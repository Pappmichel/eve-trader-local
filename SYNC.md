...
SDE/ESI/config foundation. Only classification is ported so far; everything
needing a `ProductionConfig` or Goonmetrics pricing (buy-vs-build, invention
math, build-candidate discovery, `potential_daily_profit`/`daily_movement`)
is still in "Candidates" below.
...
| `eve_trader/production/engine.py` (the rest) | `potential_daily_profit`/`daily_movement`, buy-vs-build expansion, `job_category`/`_structure_profile`, the ME/TE stacking helpers — everything past `classify_activity`, which is ported (see above). Most of it needs a `ProductionConfig` and/or Goonmetrics pricing first |
