...
class ProductionConfig:
    ...
    # Station to move materials *from* on logistics_status/
    # distribution_recommendations' Distribution section (GitHub issue #4,
    # ported from the parent's own field - a single eve-trader-local tenant
    # can genuinely run multiple characters/structures, same as the parent's
    # multi-tenant model, so this is no longer out of scope here - see
    # SYNC.md). Falls back to home_location_id when unset ("the home market
    # acts as the central warehouse" by default), but independently
    # overridable since the user may want to distribute from somewhere else -
    # see engine.distribution_recommendations.
    ...
    # needed. Independent of job_category_locations (invention isn't itself a
    # job_category bucket) - see engine.invention_logistics.
